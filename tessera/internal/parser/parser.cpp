...
{
    std::shared_ptr<tess::expression> expr;
    auto result = x3::phrase_parse(input.begin(), input.end(), tess::parser::expr, tess::parser::skip_whitespace, expr);

    return tess::tessera_script(expr);
}
