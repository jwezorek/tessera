...
namespace tess {
	namespace parser {
		...
		x3::rule<class special_func_expr_aux_, std::tuple<std::string, std::vector<expr_ptr>>> const special_func_expr_aux = "special_func_expr_aux";
		...
		auto const special_func_def = kw_<kw::sqrt>() | kw_<kw::sin>() | kw_<kw::cos>() | kw_<kw::tan>() |
			...
			kw_<kw::isosceles_triangle>() | kw_<kw::isosceles_trapezoid>() | kw_<kw::rhombus>() | kw_<kw::flip>() | kw_<kw::polygon>() | kw_<kw::join>();
...
std::tuple<tess::expr_ptr, std::string::const_iterator> tess::parser::special_expr_::parse_aux(const text_range& input) const
{
	...
	bool success = x3::phrase_parse(iter, input.end(), tess::parser::special_expr, x3::space, output);
	if (success)
		return { output, iter };
	else
