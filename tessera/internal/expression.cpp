...
namespace {
	...
	{
		if (!std::holds_alternative<tess::tile_ptr>(arg) && !std::holds_alternative<tess::patch_ptr>(arg))
			throw tess::error("attempted to flip a value that is not a tile or patch");

		std::variant<tess::tile_ptr, tess::patch_ptr> flippable_val = variant_cast(arg);
	...
	{
		if (! std::holds_alternative<tess::cluster_ptr>(arg))
			return {};

		auto* vertices = std::get<tess::cluster_ptr>(arg);
		...
		try {
			std::transform(vertices->begin(), vertices->end(), tuples.begin(),
				[&](const auto& ev)->std::tuple<tess::number, tess::number> {
					auto* pt = std::get<tess::cluster_ptr>(ev);
	...
	std::vector<special_func_def> g_special_function_definitions = {
		{
			...
			[](tess::allocator& a, const std::vector<tess::expr_value>& args)->tess::expr_value {
				...
				return tess::expr_value(a.create<tess::tile_ptr>(&a, locs) );
			}
		} , {
			...
			[](tess::allocator& a,const std::vector<tess::expr_value>& args)->tess::expr_value {
				auto patch = std::get<tess::patch_ptr>(args[0]);
...
void tess::on_expr::compile(stack_machine::stack& stack) const
{
	stack.push(
		std::make_shared<val_func_op>(
			2,
			[](allocator& a, const std::vector<expr_value>& args) -> expr_value {
				std::variant<tess::tile_ptr, tess::patch_ptr> tile_or_patch = variant_cast(args[0]);
				std::variant<tess::edge_ptr, tess::cluster_ptr> arg = variant_cast(args[1]);
				return std::visit(
